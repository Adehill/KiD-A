...
- we require a simulation to be run with maximum vertical velocity (wctrl) = 0.25 and 1 m s<sup>-1</sup>. Once again wctrl is set in the namelist for the case. Na (or Nd depending on scheme) = 50, 150 and 300 cm<sup>-3</sup> for the following configurations:
   - cond-evap case only aersol-activation (dependent on scheme), condensation and evaporation are switched on, i.e. sedimentation, collision-coalescence, breakup, etc. are switched off.
...
- As with previous tests, we would like participants to provide a tests with
   - Nd = 50, 150 and 300 cm<sup>-3</sup>
   - mu = 0.0 and 2.5
